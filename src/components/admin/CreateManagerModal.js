...
import { createManager, selectCreateManagerLoading } from "@/redux/slices/managerSlice";
import { toast } from "react-hot-toast";
import Input from "@/components/common/Input";
import PhoneInput from "@/components/common/PhoneInput";

export default function CreateManagerModal({ isOpen, onClose, onSubmit }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            // Send data in the exact format required by API
            const managerData = {
                name: formData.name,
            ...
            try {
                const result = await dispatch(createManager(managerData)).unwrap();
                toast.success("Manager created successfully!");

                // Call onSubmit callback if provided (for updating local state)
                if (onSubmit) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
            <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl transform transition-all animate-slideUp">
                ...
                <div className="relative p-6 border-b border-gray-200 bg-gradient-to-r from-teal-500 to-teal-600 rounded-t-2xl">
                    <h2 className="text-2xl font-bold text-white">Create New Manager</h2>
