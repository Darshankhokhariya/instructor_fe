...
function Manager() {
  ...
  const managers = Array.isArray(managersState?.data?.data)
    ? managersState.data.data
    : [];
  ...
  useEffect(() => {
    if (id) {
      const findManager = filteredManagers?.find((data) => data?.id === id);
