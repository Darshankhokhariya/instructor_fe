...
const Input = ({ className, error, ...props }) => (
    <input
        {...props}
        className={`w-full p-3 border rounded-xl shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 ${error ? 'border-red-500 ring-red-100' : 'border-slate-300'} ${className}`}
    />
);
...
const OtpVerification = ({ phoneNumber = "+91 12345 67890" }) => {
    ...
    const handleChange = useCallback((e, index) => {
        ...
        } else if (index === OTP_LENGTH - 1 && newOtp.join('').length === OTP_LENGTH) {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (fullOtp === '123456') { // Mock Success Condition
            setStatus('success');
            // Navigate or set next state after success animation
            setTimeout(() => console.log("OTP verified successfully!"), 1500);
    ...
    const isButtonDisabled = otp.join('').length !== OTP_LENGTH || status === 'verifying' || status === 'success';
    ...
    if (status === 'success') {
        return (
            <div className="flex flex-col items-center justify-center p-10 text-center h-[400px]">
                ...
                <h2 className="text-2xl font-bold text-teal-800 mb-2">Verification Complete!</h2>
                <p className="text-slate-500 max-w-sm">Your identity has been successfully verified. Redirecting you to the next step...</p>
            </div>
        );
    }
        ...
        <div className="flex flex-col items-center p-6 md:p-10 w-full max-w-md mx-auto bg-white rounded-2xl shadow-xl border border-slate-100">
            ...
            <div className="w-full mb-8 text-center">
                ...
                <p className="text-sm text-slate-500 mt-2">
                    A 6-digit code has been sent to <span className="font-semibold text-teal-600">{phoneNumber}</span>.
                </p>
            </div>
            ...
            <div className="w-full flex justify-center space-x-2 mb-6" >
                {otp.map((digit, index) => (
                    <Input
                        ...
                        className={`text-center text-xl font-mono h-12 w-12 border-2 ${error ? 'border-red-500' : 'focus:border-teal-600'} transition-colors duration-200`}
                        autoFocus={index === 0}
                        required
                    />
                ))}
            ...
            <div className="mt-6 w-full text-center text-sm text-slate-600 flex items-center justify-center gap-1">
                ...
                {countdown > 0 ? (
                    <span>
                        Resend in <span className="font-bold text-teal-600">{countdown}s</span>
                    </span>
                ) : (
                    <button
...
const OtpVerificationPage = () => {
    return (
        <div className="h-screen w-full bg-slate-50 flex flex-col items-center justify-center p-4">
            ...
            <button className="flex items-center mt-6 text-slate-500 hover:text-teal-600 transition-colors">
